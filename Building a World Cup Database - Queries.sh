...
echo "$($PSQL "SELECT DISTINCT(name) FROM games FULL JOIN teams on games.winner_id = teams.team_id WHERE year = 2014 AND ROUND = 'Eighth-Final' UNION SELECT DISTINCT(name) FROM games FULL JOIN teams on games.opponent_id = teams.team_id WHERE year = 2014 AND ROUND = 'Eighth-Final' ORDER BY name")"
